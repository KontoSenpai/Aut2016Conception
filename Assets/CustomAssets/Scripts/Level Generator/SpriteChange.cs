...
public class SpriteChange : MonoBehaviour {
    ...
    private Sprite blockVSimple;
    private Sprite blockFullVSimple;

    private GameObject bg;
    private GameObject[][] gridBlocks;
    private int round;

    public void SetSelection(int r)
    {
        round = r;
        Destroy(bg);
        bg = new GameObject();
        ...
        else if(round == 2)
        {
            ...
            background.transform.parent = bg.transform;
            blockVSimple = blockCave;
            blockFullVSimple = blockFullCave;
        }
        else if(round == 3)
        {
            GameObject background = Instantiate(backgrounds[2], new Vector3(10, 9.5f, 0), transform.rotation) as GameObject;
            background.transform.parent = bg.transform;
            blockVSimple = blockColiseum;
            blockFullVSimple = blockFullColiseum;
        }
    }
    ...
    public void ChangeSprites(GameObject[][] grid, GameObject[] rows, GameObject[] bottom)
    {
        ...
        else
        {
            for (int x = 0; x < gridBlocks.Length; x++)
            {
                ...
                {
                    if (gridBlocks[x][y] != null && gridBlocks[x][y].name.Contains("Half"))
                        gridBlocks[x][y].GetComponentInChildren<SpriteRenderer>().sprite = blockVSimple;
                    else if (gridBlocks[x][y] != null && gridBlocks[x][y].name.Contains("Full"))
                        gridBlocks[x][y].GetComponentInChildren<SpriteRenderer>().sprite = blockFullVSimple;
                }
            }
        }
...
*/
    ...
    */
    ...
    public Sprite GetSpriteSimple() { return blockVSimple; }

    public Sprite GetSpriteFull(int index) { return blocksFullV[index]; }
    public Sprite GetSpriteFullSimple() { return blockFullVSimple; }
}
